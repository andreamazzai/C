...
int main(void) {
	...
	(void) printf("\nqualcosa"); cast a void del risultato della printf, che sarebbe il numero di caratteri stampati
	return 0;
}
*/
...
int main (void) {
	...
	if (isprime(numero) == true)
		printf("\n\nIl numero %d è primo", numero);
	else
		printf("\n\n159Il numero %d non è primo", numero);
	return 0;
}
*/

/*
// PROGRAMMA average.c pagina 199
int main(void) {
	double x, y, z;
	printf("\n\nDigita 3 numeri double separati da uno spazio: ");
	scanf("%lf %lf %lf", &x, &y, &z);
	//~ double average(double parametroA, double parametroB); // dichiarazione di funzione che definisco dopo l'invocazione; secondo KNK pagina 200 precede il main(). Questo è un prototipo di funzione.
	double average(double, double); // come sopra, ma senza nome dei parametri	
	printf("\nMedia di %g e %g = %g", x, y, average(x, y));
	printf("\nMedia di %g e %g = %g", x, z, average(x, z));
	printf("\nMedia di %g e %g = %g", y, z, average(y, z));
	return 0;
...
int somma_elementi(int [], int);		// prototipo
int main(void) {						// main
	...
	printf("\nSomma di %d elementi del vettore = %d", SIZE(elementi), totale);
	return 0;
}
int somma_elementi(int daSommare[], int len) {		// funzione.. il nome dell'array lo posso definire al volo
	len--;
	int sum = 0;
	while (len >= 0) {
	sum += daSommare[len];
...
/*
// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// PROGRAMMA pagina 204 - duplicato /1 ma con ** modifica elementi del vettore nella funzione invocata **
	// somma di elementi di vettore passato come argomento
#define SIZE(array) (sizeof array / sizeof(array[0]))
int somma_elementi(int [], int);		// prototipo chiamata funzione
int main(void) {						// main
	printf("\n\n*** Somma di tutti gli elementi di un vettore ***");
	int elementi[10] = {0};
	int sum = 0;
	...
	printf("\n1) Somma di %d elementi inizializzati a 0 in corpo main = %d", SIZE(elementi), sum);
	int totale = 0;
	srand((unsigned) time(NULL));
	for (int i = 0; i < SIZE(elementi); i++) {
		totale += elementi[i] = rand() % 20;
	}
	printf("\n2) Somma di %d elementi inizializzati con rnd = %d", SIZE(elementi), totale);
	totale = somma_elementi(elementi, SIZE(elementi)); // invoco la funzione che assegnerà 'return sum' a totale
	printf("\n4) Somma di %d elementi restituita dalla funzione = %d", SIZE(elementi), totale);
	...
	printf("\n5) Somma di %d elementi modificati nella funzione = %d", SIZE(elementi), sum);
	return 0;
}
int somma_elementi(int daSommare[], int len) {		// funzione.. il nome dell'array lo posso definire al volo
	int len2 = len--;
	...
	printf("\n3) Somma degli elementi calcolata nella funzione = %d", sum);
	// metto nuovi elementi nel vettore
	for (int i = 0; i < len2; i++)
		sum2 += daSommare[i] = rand() % 20;
	printf("\n3a) Somma degli elementi inseriti nella funzione = %d", sum2);
...
int main(void) {						// main
	...
	printf("\n4) Somma di %d elementi restituita dalla funzione = %d", len * len, totale);
	sum = 0;
	for (int i = 0; i < len; i++) {
		for (int j = 0; j < len; j++) {
			sum += elementi[i][j];
...
int somma_elementi_vettore(int daSommare[][LUNGHEZZA], int len) {		// vettore multidimensionale: posso omettere solo prima dimensione
	...
	printf("\n3) Somma di %d elementi dentro alla funzione = %d", len * len, sum);
	return sum; // risponde alla chiamata passando sum
}
*/
...
// PROGRAMMA pagina 206 - vettore lunghezza variabile monodimensionale
	// somma di elementi di vettore passato come argomento
#define SIZE(array) (sizeof array / sizeof(array[0]))
int somma_elementi_vettore(int len, int array[len]);		// prototipo
int main(void) {						// main
	printf("\n\nSomma di tutti gli elementi di un vettore");
	int elementi[10] = {0};
	int totale;
...
int main(void) {	// main
	...
	printf("\nSomma di %d elementi del vettore = %d", rows * cols, totale);
...
int split(int low, int high, int vettore[high]) {
	int partElement = 0;
	printf("\n\n---------------------------");
...
int main(void){
	...
	int numeroInput;
	scanf("%d", &numeroInput);
	printf("Il numero %d è composto da %d cifre.", numeroInput, num_digits(numeroInput));
}
*/
...
int main(void) {
	...
	printf("La %da cifra di %d è %d.", cifra, numero, nEsima(numero, cifra));
}
*/
